node{
  ...
  stage('Docker Build, Push'){
    withCredentials([usernameColonPassword(credentialsId: 'art4lab0-docker-deploy', variable: 'docker_deploy')]) {
            ...
            sh "docker tag ${ImageName}:${imageTag} art4lab0.labs.mastercard.com:6565/artifactory/docker-internal/test/${ImageName}:${imageTag}"
            sh "docker push art4lab0.labs.mastercard.com:6565/artifactory/docker-internal/test/${ImageName}:${imageTag}"
        }

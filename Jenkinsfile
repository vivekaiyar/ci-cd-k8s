node{
  ...
  stage('Docker Build, Push'){
    //withDockerRegistry([credentialsId: "Docker Registry Auth", url: 'art4lab0.labs.mastercard.com:6565']) {
      ...
      sh "docker build -t ${ImageName}:${imageTag} ."
      sh "docker tag ${ImageName}:${imageTag} art4lab0.labs.mastercard.com/artifactory/xray-jenkins-npm/${ImageName}:${imageTag}"
      sh "docker push art4lab0.labs.mastercard.com/artifactory/xray-jenkins-npm/${ImageName}:${imageTag}"
